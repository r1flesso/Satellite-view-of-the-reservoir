...
## Примечания
1. Перед запуском моделей градиентного бустинга проверьте, что Вы можете использовать графический процессор (с помощью пакета cuda), в противном случае вместо 'cuda' укажите 'cpu' в параметре device
2. Вам обязательно нужно создать виртулаьное окружение для PyTorch
