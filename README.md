...
В рамках проекта я обучил несколько алгоритмов классификации с разными комбинациями гиперпараметров. Для их подкручивания использовал библиотеку Optuna. Все метрики, гиперпараметры, модели и т.д. логировал/регистрировал в MLflow, чтобы потом системно проанализировать. Из всех обученных моделей выбрал ту, что показала лучшие результаты, и "докрутил" ее веса, а также прописал кастомный словарь весов. В итоге завернул готовую модель в Docker-образ — чтобы показать полный жизненный цикл ML-решения: от экспериментов до контейнеризации
...
2. Вам обязательно нужно создать виртуальное окружение для PyTorch
